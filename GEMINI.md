...
- **NEVER change the model** unless explicitly asked. This project pins `gemini-2.5-flash`: Vertex rejects AI Studio aliases like `gemini-flash-latest` (see LESSONS.md).
- **Model 404 errors**: Fix `GOOGLE_CLOUD_LOCATION` (e.g., `global` instead of `us-central1`), not the model name.
- **ADK tool imports**: Import the tool instance, not the module: `from google.adk.tools.load_web_page import load_web_page`
- **Run Python directly** (no uv on this machine): `python script.py` inside the `reversa` pyenv env. Run `make install` first.
